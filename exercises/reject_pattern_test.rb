...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_words_ending_in_ing
    ...
    selected = []
    words.each do |word|
      selected << word unless word.reverse[0..2] == "gni"
    end
  ...
  def test_remove_numbers
    ...
    elements.each do |elem|
      not_numbers << elem unless elem.instance_of?(Integer) || elem.instance_of?(Float)
    end
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each do |elem|
      not_numbers << elem unless elem.instance_of?(Float)
    end
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each do |elem|
      remaining << elem unless elem.instance_of?(Array)
    end
  ...
  def test_remove_hashes
    ...
    remaining = []
    elements.each do |elem|
      remaining << elem unless elem.instance_of?(Hash)
    end
